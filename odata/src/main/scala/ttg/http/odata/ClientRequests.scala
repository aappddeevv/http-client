...
trait ClientRequests[F[_]] {
  ...
  def mkGetListRequest(url: String, opts: Option[RequestOptions] = None) =
    HttpRequest[F](Method.GET, url, headers = toHeaders(opts), body=emptyBody)

  def mkCreateRequest[B](entitySet: EntitySetName, body: B, opts: Option[RequestOptions] = None)(
    ...
    HttpRequest[F](Method.POST, s"/${entitySet.asString}", body = ent,
      headers = toHeaders(opts) ++ xtras)
  }
  ...
  def mkDeleteRequest(entitySet: EntitySetName, key: ODataId, opts: Option[RequestOptions] = None) = {
    ...
    HttpRequest[F](Method.DELETE, s"/${entitySet.asString}(${renderId(key)})",
      headers = toHeaders(opts) ++ etag,
      body = emptyBody)
  }

  def mkGetOneRequest(url: String, opts: Option[RequestOptions] = None) = {
    val etag = opts.flatMap(_.version.map(etag =>
      HttpHeaders("If-None-Match" -> etag))).getOrElse(HttpHeaders.empty)
    HttpRequest[F](Method.GET, url, headers = toHeaders(opts) ++ etag, body=emptyBody)
  }
  ...
  def mkUpdateRequest[B](entitySet: String,
    ...
    HttpRequest(
      method = Method.PATCH,
      path = s"${base.getOrElse("")}/$entitySet($id)",
      headers = toHeaders(options) ++ h1 ++ h2 ++ h3 ++ xtra ++ mustHave,
      body = ent)
  }
